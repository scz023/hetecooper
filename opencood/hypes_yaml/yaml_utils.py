...
def load_yaml(file, opt=None, cooper=None):
    """
    ...
    if cooper and opt and opt.model_dir:
        file = os.path.join(opt.model_dir_cooper, 'config.yaml')
...
def load_voxel_params(param):
    ...
    if 'model' in param:
        param['model']['encoder']['args']['W'] = anchor_args['W']
        param['model']['encoder']['args']['H'] = anchor_args['H']
        param['model']['encoder']['args']['D'] = anchor_args['D']

        # see the feature input to the fuse_net
        if 'fuse' in param['model']:
            param['model']['fuse']['args']['H'] = anchor_args['H']
            param['model']['fuse']['args']['W'] = anchor_args['W']
...
def load_point_pillar_params(param):
    ...
    param['model']['encoder']['args']['point_pillar_scatter']['grid_size'] = grid_size
...
def load_second_params(param):
    ...
    grid_size = (np.array(cav_lidar_range[3:6]) - np.array(
        cav_lidar_range[0:3])) / \
                np.array(voxel_size)
    grid_size = np.round(grid_size).astype(np.int64)
    param['model']['encoder']['args']['grid_size'] = grid_size
