...
__device__ float overlap_ratiod(float pos_ego_x, float pos_ego_y, float len_ego_x, float len_ego_y, float pos_neb_x, float pos_neb_y, float len_neb_x, float len_neb_y) {
    // 计算矩形ego的左上角和右下角坐标
...
__device__ float dis(float pos_ego_x, float pos_ego_y, float pos_neb_x, float pos_neb_y) {
    float delta_x = pos_ego_x - pos_neb_x;
    float delta_y = pos_ego_y - pos_neb_y;
    return sqrt(delta_x*delta_x + delta_y*delta_y);
}

__device__ float sin(float pos_ego_x, float pos_ego_y, float pos_neb_x, float pos_neb_y) {
    float delta_x = pos_ego_x - pos_neb_x;
    float delta_y = pos_ego_y - pos_neb_y;
    float dist = sqrt(delta_x*delta_x + delta_y*delta_y);
    if (dist == 0) return 0;
    return abs(delta_y) / dist;
}

__device__ float cos(float pos_ego_x, float pos_ego_y, float pos_neb_x, float pos_neb_y) {
    float delta_x = pos_ego_x - pos_neb_x;
    float delta_y = pos_ego_y - pos_neb_y;
    float dist = sqrt(delta_x*delta_x + delta_y*delta_y);
    if (dist == 0) return 1;
    return abs(delta_x) / dist;
}
    ...
    )
{
    ...
    // printf("%d\n", max_trace);
    build_edge_kernel<<<grid_size, block_size>>>(
