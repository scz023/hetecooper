...
from opencood.models.fuse_modules.hete_where2comm_fuse import Where2comm


class HetePointPillarWhere2comm(nn.Module):
    def __init__(self, args):
        super(HetePointPillarWhere2comm, self).__init__()
        self.state = {
            "ego": True, # 是否为协作车 True False
            "ITHP": "train_homo" # train_homo train_proj inference_hete
        }
        ...
        self.fusion_net = Where2comm(args['where2comm_fusion'], self.state)
    ...
    def forward(self, data_dict, neb_info=None):
        ...
        # 若在训练线形层过程中, 返回N, C, H', W': [N, 256, 48, 176]
        if self.state["ITHP"] == "train_proj":
        ...
        # 若在异构推理过程中且为neb, 返回协作所需的相关数据
        if (not self.training) and self.state["ITHP"] == "inference_hete" and self.state['ego']==False:
            neb_info = {
                "model_name" : "PointPillar",
                "neb_features_2d" : fused_feature,
