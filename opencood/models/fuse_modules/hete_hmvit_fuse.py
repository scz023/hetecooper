...
from opencood.models.fuse_modules.swap_fusion_modules_hete import \
    HeteSwapFusionEncoder
from opencood.models.fuse_modules.fuse_utils import regroup
from opencood.models.sub_modules.proj_encoder import ProjEncoder


class HeteHmvitFuse(nn.Module):
    def __init__(self, args):
        super(HeteHmvitFuse, self).__init__()
        ...
        args['num_types'] = len(self.proj_dict.keys())
        # self.fusion_net = HeteSwapFusionEncoder(args)
        # 当智能体数量为2时, hm-vit与cobvet模型结构相同. 
        # 完整hm-vit方法的实现将在后续补充
    ...
    def forward(self, middle_info, neb_middle_info=None):
        ...
        if neb_middle_info:
            ...
            # 直接为不同类型的智能体分别训练atten参数, 而无需进行特征映射
            ...
            # if neb_middle_info['model_name'] == 'Second':
            #     type = 1
            # if neb_middle_info['model_name'] == 'VoxelNet':
            #     type = 2
        ...
        # fused_feature = self.fusion_net(regroup_feature, com_mask, type)
