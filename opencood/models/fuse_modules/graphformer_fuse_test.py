...
class RatioMulXyEdgeEnc(nn.Module):
    ...
    def forward(self, values):
        ...
        # return xy_pe
        ratio = (values[:,1] + 1).unsqueeze(1).repeat(1, C)
        e = xy_pe * ratio

        e = e / math.sqrt(C) / 100
        # e = values[:,2:4] * (values[:,1].unsqueeze(1) + 1)
        # e = values[:,1:4]
        # e = self.out(e)
...
class Graphformer(nn.Module):
    def __init__(self, args):
        ...
        self.split_atten = False
        
        if 'split_atten' in args.keys():
            self.split_atten = True
        ...
        for k in range(len(self.window_size)):
            ...
            self.scales.append(self.layers)
    ...
    def forward(self, ego, neb, neb_confidence_map, neb_point_cloud_range):
        ...
        # 距离位置编码
        ego_init = self.dis_enc(ego, self.point_cloud_range)
        neb_init = self.dis_enc(neb, neb_point_cloud_range)  
        # 角度位置编码 
        ...
        for window_size in self.window_size:
            ...
            window_size_neb = window_size
            while window_size_neb >0: 
                # 调整window_size_neb大小, 使得neb尺寸不小于ego
                scaled_size_neb, _ = sc_padding(neb_init, window_size_neb)
                window_size_neb = window_size_neb - 1
                if (torch.Tensor(scaled_size_neb) >= torch.Tensor(scaled_size_ego)).all().item():
                    break
            window_size_neb = window_size_neb + 1          
            
            
            
            # 当ego特征密度小于neb时, 提取不同尺度的neb特征
            scale_fuse = []
            while True:
                scaled_size_neb, padding_neb = sc_padding(neb_init, window_size_neb)
                neb = F.pad(neb_init, padding_neb)
                # print(neb.size())
                
                if window_size_neb != window_size:
                    # 窗口大小不一致, 插值/池化操作统一
                    neb = rearrange(neb, 'c (h_sc ws_h) (w_sc ws_w)  -> c (h_sc w_sc) ws_h ws_w',
                            ws_h = window_size_neb, ws_w = window_size_neb)
                    neb = F.interpolate(neb, (window_size, window_size), mode='bilinear', align_corners=True)
                    neb = rearrange(neb, 'c nodes ws_h ws_w -> nodes (ws_h ws_w) c')
                else:
                ...
                if neb_nonzero_indices.size()[0] > 0:
                    ...
                    # neb feature interplation to ego for fuse here
                ...
                scale_fuse.append(ego)
                # ego合并完数据粒度比其小的neb特征后 退出循环(特征表征真实空间一致, 即粒度小的特征尺寸也越大),
                # 继续增大数据粒度, 开始融合特征
                if (torch.Tensor(scaled_size_neb) <= torch.Tensor(scaled_size_ego)).all().item():
                    break
                    # neb = h[div_idx:]
            ...
            # neb avg here
            scale_fuse = torch.stack(scale_fuse)
            ego = torch.mean(scale_fuse, dim=0)
    ...
    def build_graph(self, ego, neb, search_range, neb_nonzero_indices, neb_point_cloud_range):
        ...
        delta_x = delta_xyz * values[:, 2] / (dis + 1e-7)
        delta_y = delta_xyz * values[:, 3] / (dis + 1e-7)
