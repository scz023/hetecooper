...
class IntermediateFusionDatasetDAIRWithneb(Dataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True, params_neb = None):
        self.params = params
        self.params_neb = params_neb
        ...
        if self.params_neb:
            self.pre_processor_neb = build_preprocessor(params_neb['preprocess'],
                                                train)
            self.post_processor_neb = post_processor.build_postprocessor(
                params_neb['postprocess'],
                train)
    ...
    def retrieve_base_data(self, idx):
        ...
        data : dict
            ...
            each cav.
        """
        # data为字典，键值ego、infrastructure对应的键值分别自定义为0、1
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, ego_pose_clean, ego_keypoints, ego_allpoints, idx, neb = False):
        """
        ...
        if neb:
            ...
            lidar_np = mask_points_by_range(lidar_np,
                                            self.params['preprocess'][
                                                'cav_lidar_range'])
        if neb:
            processed_lidar = self.pre_processor_neb.preprocess(lidar_np)
        else:
    ...
    def get_item_separately(self, idx, base_data_dict, neb = False):
        ...
        processed_features = [] # features of cav and infrastructure
        object_stack = []
        object_id_stack = []
        ...
        # loop over all CAVs to process information, cav_id_list: [0, 1]，即retrieve_base_data中赋的值
        ...
        for cav_id in cav_id_list:
            ...
            # no data in dair-v2x setting
            # onlt use for adaptation the input in v2x-vit, 
            # set default,  
            # # normalize veloccity by average speed 30 km/h
            velocity.append(float(30/30))
            time_delay.append(float(0)) 
            spatial_correction_matrix.append(
                np.eye(4))
            # in dair-v2x, cav_id = 1 , means infra
        ...
        if self.kd_flag:
            ...
            stack_lidar_np = mask_points_by_range(stack_lidar_np,
                                        self.params['preprocess'][
                                            'cav_lidar_range'])
            if neb:
                stack_feature_processed = self.pre_processor_neb.preprocess(stack_lidar_np)
            else:
        ...
        if neb:
            ...
            anchor_box = self.post_processor_neb.generate_anchor_box()

            # generate targets label
            label_dict = \
                self.post_processor_neb.generate_label(
        ...
        return processed_data_dict['ego']
    ...
    def __getitem__(self, idx):
        ...
        processed_data_dict = OrderedDict()
        processed_data_dict['ego'] = self.get_item_separately(idx, base_data_dict)
        if self.params_neb:
            processed_data_dict['neb'] = self.get_item_separately(idx, base_data_dict, True)
    ...
    def collate_batch_train(self, batch):
        ...
        output_dict['ego'] = self.collate_batch_single(batch)

        if self.params_neb:
            output_dict['neb'] = self.collate_batch_single(batch, True)
    
        return output_dict
    
    def collate_batch_single(self, batch, neb = False):
        # Intermediate fusion is different the other two
        output_dict = {'ego': {}}
        ...
        if neb:
            ...
            self.pre_processor_neb.collate_batch(merged_feature_dict)
            # [2, 3, 4, ..., M], M <= max_cav
            ...
            label_torch_dict = \
                self.post_processor_neb.collate_batch(label_dict_list)
            label_torch_dict_single_v = \
                self.post_processor_neb.collate_batch(label_dict_list_single_v)
            label_torch_dict_single_i = \
                self.post_processor_neb.collate_batch(label_dict_list_single_i)
        ...
        if neb:
            if self.params_neb['preprocess']['core_method'] == 'SpVoxelPreprocessor' and \
    ...
    def collate_batch_test(self, batch):
        ...
        if self.params_neb:
            # check if anchor box in the batch
            if batch[0]['neb']['anchor_box'] is not None:
                output_dict['neb'].update({'anchor_box':
                    torch.from_numpy(np.array(
                        batch[0]['neb'][
                            'anchor_box']))})
            ...
            output_dict['neb'].update({'transformation_matrix':
                                        transformation_matrix_torch,
                                        'transformation_matrix_clean':
                                        transformation_matrix_clean_torch,})

            output_dict['neb'].update({
                "sample_idx": batch[0]['neb']['sample_idx'],
                "cav_id_list": batch[0]['neb']['cav_id_list']
            })
