...
def inference_intermediate_fusion(batch_data, model, dataset, neb_info = None):
    """
    Model inference for early fusion.
    ...
    # only output ego detect results
    none_ego = []
    ...
    output_dict = OrderedDict()
    cav_content = batch_data['ego']
    if neb_info is not None:
        output_dict['ego'] = model(cav_content, neb_info)
    else:
        output_dict['ego'] = model(cav_content)
...
def inference_intermediate_fusion_withcomm(batch_data, model, dataset, neb_info = None):
    """
    Model inference for early fusion.
    ...
    comm_rates = output_dict['ego']['com']
