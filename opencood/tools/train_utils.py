...
def load_saved_model(saved_path, model, epoch=None):
    ...
    if os.path.exists(os.path.join(saved_path, 'net_latest.pth')):
        ...
        # load_state_dict需要模型结构一致才可加载
        # 第一个循环从state_dict中去掉model_state_dict中没有的键值
        # 第二个循环为state_dict补上所确实的model_state_dict中的键值，并初始化为model_state_dict自带的随机值
        # 最终实现model_state_dict和state_dict结构的统一
        for k in state_dict:
        ...
        # model loaded has been trained, should strart from initial_epoch + 1
        return initial_epoch, model
...
def load_saved_model_proj(saved_path, model, epoch=None):
    """
    Load saved model if exiseted
    ...
    def findLastCheckpoint(save_dir):
        file_list = glob.glob(os.path.join(save_dir, 'net_proj_epoch*.pth'))
        if file_list:
            epochs_exist = []
            for file_ in file_list:
                result = re.findall(".*net_proj_epoch(.*).pth.*", file_)
                epochs_exist.append(int(result[0]))
            initial_epoch_ = max(epochs_exist)
        else:
            initial_epoch_ = 0
    ...
    if os.path.exists(os.path.join(saved_path, 'net_latest_proj.pth')):
        model.load_state_dict(torch.load(
            os.path.join(saved_path,
                         'net_latest_proj.pth')))
        return 100, model
        ...
        if initial_epoch > 0:
            print('resuming proj by loading epoch %d' % initial_epoch)
        else: 
            return initial_epoch, model
        
        state_dict_ = torch.load(os.path.join(saved_path, 'net_proj_epoch%d.pth' % initial_epoch))
        state_dict = {}
        # convert data_parallal to model
        for k in state_dict_:
            if k.startswith('module') and not k.startswith('module_list'):
        ...
        # load_state_dict需要模型结构一致才可加载
        # 第一个循环从state_dict中去掉model_state_dict中没有的键值
        # 第二个循环为state_dict补上所缺失的model_state_dict中的键值，并初始化为model_state_dict自带的随机值
        # 最终实现model_state_dict和state_dict结构的统一
        for k in state_dict:
            if k in model_state_dict:
                if state_dict[k].shape != model_state_dict[k].shape:
            ...
            #     print('Drop parameter {}.'.format(k))
        for k in model_state_dict:
            if not (k in state_dict) and 'proj' in k:
                print('No param {}.'.format(k))
                state_dict[k] = model_state_dict[k]
        model.load_state_dict(state_dict, strict=False)
        return initial_epoch, model
...
def create_loss_proj(hypes):
    """
    Create the proj loss function based on the given proj loss name.
    ...
    criterion : opencood.object
        The loss function.
    """
    loss_func_name = hypes['proj_train']['loss']['core_method']
    # loss_func_config = hypes['loss']['args']
...
def setup_optimizer_proj(hypes, model):
    """
    Create optimizer corresponding to the yaml file
    ...
    model : opencood model
        The pytorch model
    """
    method_dict = hypes['proj_train']['optimizer']
    optimizer_method = getattr(optim, method_dict['core_method'], None)
    print('optimizer method is: %s' % optimizer_method)
...
def setup_lr_schedular_proj(hypes, optimizer, n_iter_per_epoch):
    """
    Set up the learning rate schedular.
    ...
    optimizer : torch.optimizer
    """
    lr_schedule_config = hypes['proj_train']['lr_scheduler']
...
def setup_optimizer_fuse(hypes, model):
    """
    Create optimizer corresponding to the yaml file
    ...
    model : opencood model
        The pytorch model
    """
    method_dict = hypes['fuse_train']['optimizer']
    optimizer_method = getattr(optim, method_dict['core_method'], None)
    print('optimizer method is: %s' % optimizer_method)
...
def setup_lr_schedular_fuse(hypes, optimizer, n_iter_per_epoch):
    """
    Set up the learning rate schedular.
    ...
    optimizer : torch.optimizer
    """
    lr_schedule_config = hypes['fuse_train']['lr_scheduler']
